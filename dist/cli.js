...
import { combineArticleSentenceCoreV2, dumpArticleSentenceCoreCombineTupleV2Json, } from "./article-word-pipeline/v2/articleSentenceCoreCombine.js";
...
import { ArticleCoreWordsDataSchema, ArticleCoreWordsNomalizedDataSchema, ArticleMemorySentencesDataSchema, } from "./article-word-pipeline/v2/schemas.js";
...
import { runStoryWordSentenceBucketsFromDb, runStoryWordSentencePipelineFromDb, } from "./dreaming-pipeline/story-word-sentence-pipeline/index.js";
...
program
    ...
    try {
        ...
        exitValidation(e, "Failed to parse ArticleCoreWordsData (expected core_words string array)");
    }
...
program
    ...
    .requiredOption("--from-normalized-json <path>", "article-core-words-normalize JSON path")
    .action((opts) => {
    let sentences;
    let normalized;
    try {
        ...
        exitValidation(e, "Failed to parse ArticleMemorySentencesData (expected sentences: [{sentence}])");
    }
    try {
        normalized = ArticleCoreWordsNomalizedDataSchema.parse(JSON.parse(readUtf8(opts.fromNormalizedJson)));
    }
    catch (e) {
        exitValidation(e, "Failed to parse ArticleCoreWordsNomalizedData (expected nomalized array)");
    }
...
program
    ...
    .description("Sample words → linked sentences; output sentences JSON (short-term pool in full, then weighted long-term sample)")
    .requiredOption("--db <path>", "SQLite database path")
    .option("--fraction <n>", "Fraction of words table to sample (default 0.2)")
    .option("--long-term-fraction <n>", "Fraction for non-short-term sentence pool (default: same as --fraction)")
    .action((opts) => {
    try {
        const fraction = opts.fraction !== undefined && opts.fraction !== ""
            ? Number.parseFloat(opts.fraction)
            : 0.2;
        const longTermFraction = opts.longTermFraction !== undefined && opts.longTermFraction !== ""
        ...
        const out = extractMemorySentencesByWordSample(resolvePath(opts.db), {
            ...
            longTermFraction: longTermFraction !== undefined && Number.isFinite(longTermFraction)
                ? longTermFraction
                : undefined,
        });
        printJson(out);
...
async function runStoryWordSentenceBucketsCli(opts) {
    const rawMaxWords = opts.maxWords !== undefined && opts.maxWords !== ""
        ? Number.parseInt(opts.maxWords, 10)
        : 10;
    const maxWords = Number.isFinite(rawMaxWords) && rawMaxWords > 0 ? rawMaxWords : 10;
    const rawFraction = opts.fraction !== undefined && opts.fraction !== ""
        ? Number.parseFloat(opts.fraction)
        : 0.2;
...
async function runStoryWordSentencePipelineCli(opts) {
    ...
    const rawFraction = opts.fraction !== undefined && opts.fraction !== ""
        ...
        : 0.2;
    const fraction = Number.isFinite(rawFraction) && rawFraction > 0 ? rawFraction : 0.2;
    const rawMinRuns = opts.minRuns !== undefined && opts.minRuns !== ""
        ? Number.parseInt(opts.minRuns, 10)
        : undefined;
    const minRuns = rawMinRuns !== undefined && Number.isFinite(rawMinRuns) && rawMinRuns > 0
        ? rawMinRuns
        : undefined;
    const rawMaxRuns = opts.maxRuns !== undefined && opts.maxRuns !== ""
        ? Number.parseInt(opts.maxRuns, 10)
        : undefined;
    const maxRuns = rawMaxRuns !== undefined && Number.isFinite(rawMaxRuns) && rawMaxRuns > 0
        ? rawMaxRuns
        : undefined;
    ...
    process.stderr.write(`[memok-ai] story-word-sentence-pipeline: plannedRuns=${out.plannedRuns} (range ${out.minRuns}–${out.maxRuns})\n`);
    printJson(out);
}
async function runDreamingPipelineCli(opts) {
    ...
    process.stderr.write(`[memok-ai] dreaming-pipeline: predream done; storyWordSentencePipeline plannedRuns=${out.storyWordSentencePipeline.plannedRuns} (${out.storyWordSentencePipeline.minRuns}–${out.storyWordSentencePipeline.maxRuns})\n`);
    printJson(out);
}
program
    .command("dreaming-pipeline")
    .description("Run predream-decay (duration decay + short-term cleanup), then story-word-sentence-pipeline; stdout is merged JSON")
    .requiredOption("--db <path>", "SQLite database path")
    .option("--max-words <n>", "Max words to sample from words table per story stage (default 10)")
    .option("--fraction <n>", "Sampling fraction for sentence vs normal_words relevance (default 0.2)")
    .option("--min-runs <n>", "Minimum random pipeline runs for story-word-sentence (inclusive, default 3)")
    .option("--max-runs <n>", "Maximum random pipeline runs for story-word-sentence (inclusive, default 5)")
    .action(async (opts) => {
    try {
...
program
    ...
    .description("Predream: decrement sentences.duration globally; short-term rows with duration<=0 become long-term if weight>=7 else deleted; stdout JSON report")
    .requiredOption("--db <path>", "SQLite database path")
    .action((opts) => {
    try {
...
program
    ...
    .description("Full dreaming bucket pass: sample words, story, score buckets, dual link writes, prune orphan normal_words, merge orphan sentences; stdout JSON includes orphanSentenceMerge")
    .requiredOption("--db <path>", "SQLite database path")
    .option("--max-words <n>", "Max words from words table when generating story (default 10)")
    .option("--fraction <n>", "Sampling fraction for sentence vs normal_words relevance (default 0.2)")
    .action(async (opts) => {
    try {
...
program
    ...
    .description("Run multiple full story-word-sentence-buckets passes in sequence; run count random between --min-runs and --max-runs (default 3–5); stdout is aggregate JSON only")
    .requiredOption("--db <path>", "SQLite database path")
    .option("--max-words <n>", "Max words from words table per story round (default 10)")
    .option("--fraction <n>", "Per-round sampling fraction for sentence vs normal_words relevance (default 0.2)")
    .option("--min-runs <n>", "Random run count lower bound (inclusive, default 3)")
    .option("--max-runs <n>", "Random run count upper bound (inclusive, default 5)")
    .action(async (opts) => {
    try {
...
program
    .command("harden-db")
    .description("Remove invalid/duplicate links and ensure relationship indexes and unique constraints")
    .requiredOption("--db <path>", "SQLite database path")
    .action((opts) => {
    try {
...
program
    ...
    .requiredOption("--db <path>", "SQLite database path")
    .option("--as-of <YYYY-MM-DD>", "Optional date (YYYY-MM-DD) passed as today to import")
    .action((opts) => {
    try {
