...
export function registerMemokPluginRuntime(api, ctx) {
    ...
    if (pluginCfg.dreamingPipelineScheduleEnabled === true) {
        if (isMemokSetupCliRun()) {
            ...
            const dreamingTz = typeof pluginCfg.dreamingPipelineTimezone === "string" &&
    ...
    const runSave = async (dedupeKey, text, source) => {
        ...
        api.on("before_prompt_build", (_event, ctx) => {
            try {
                ...
                if (useSkillHint) {
                    return {
                        appendSystemContext,
                        prependContext: "（memok）完整候选在**系统上下文**定界块内；同轮再抽样请调 `memok_recall_candidate_memories`；采用后请 `memok_report_used_memory_ids`。",
                    };
                }
    ...
    if (memoryInjectEnabled) {
        const recallDescription = memoryRecallMode === "prepend"
                    ...
                    try {
                        ...
                        if (r.kind === "empty") {
                            return {
                                ...
                                details: { sentenceIds: [], empty: true },
                            };
                        }
                        if (r.truncated) {
                        ...
                        return {
                            content: [{ type: "text", text: `召回失败：${msg}` }],
                            details: { error: true },
                        };
                    }
                },
    ...
    if (memoryInjectEnabled) {
        api.registerTool((toolCtx) => {
            const reportDescription = memoryRecallMode === "prepend"
                ...
                async execute(_toolCallId, params) {
                    ...
                    if (sentenceIds.length > 0 && allowedSet) {
                        ...
                        if (outsiders.length > 0) {
                            api.logger?.warn?.(`[memok-ai] memok_report_used_memory_ids: 部分 id 不在本轮候选内: ${outsiders.join(", ")}`);
                        }
                    }
                    let updatedCount = 0;
                    ...
                    const text = sentenceIds.length === 0
