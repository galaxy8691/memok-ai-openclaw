...
export function isValidDailyAt(v) {
    return /^(?:[01]\d|2[0-3]):[0-5]\d$/.test(v.trim());
}
function toYes(v, fallback) {
...
async function pickByNumber(ask, title, rows, defaultIndex) {
    while (true) {
...
export async function promptMemokSetupAnswers() {
    const rl = createInterface({ input, output });
    const ask = (q) => rl.question(q);
    try {
        const pIdx = await pickByNumber(ask, "选择 LLM 供应商：", PROVIDERS.map((p) => p), 0);
        const llmProvider = PROVIDERS[pIdx];
        let llmModelPreset;
        if (llmProvider !== "inherit" && llmProvider !== "custom") {
            const presets = PRESET_BY_PROVIDER[llmProvider];
            const mIdx = await pickByNumber(ask, "选择模型预设（推荐非 reasoning/think）：", presets, 0);
            llmModelPreset = presets[mIdx];
        }
        const llmModel = clean(await ask(`可选：手填 llmModel（留空=沿用上一步预设${llmModelPreset ? `：${llmModelPreset}` : ""}）: `));
        const llmApiKey = clean(await ask("可选：API Key（留空则沿用环境变量）: "));
        const llmBaseUrl = llmProvider === "custom"
            ? clean(await ask("custom 模式请填写 Base URL（如 https://api.xxx.com/v1）: "))
            : undefined;
        const dreamingOn = toYes(await ask("是否启用 dreaming 定时任务？(Y/n): "), true);
        let dreamingPipelineDailyAt;
        let dreamingPipelineTimezone;
        if (dreamingOn) {
            while (true) {
...
export function mergeMemokSetupToConfig(cfg, answers) {
    const root = cfg ?? {};
    const plugins = root.plugins ?? {};
    const entries = plugins.entries ?? {};
    const slots = plugins.slots ?? {};
    const curEntry = entries["memok-ai"] ?? {};
    const curCfg = curEntry.config ?? {};
    const nextPluginCfg = {
        ...curCfg,
    ...
    if (answers.dreamingPipelineScheduleEnabled === true &&
