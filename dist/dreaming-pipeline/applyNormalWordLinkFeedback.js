 ...
 */
export function applyNormalWordLinkFeedback(dbPath, input) {
    ...
    try {
        ...
        const runTx = db.transaction(() => {
            ...
            if (plusIds.length > 0) {
                const sel = db.prepare(`SELECT 1 AS ok FROM word_to_normal_link WHERE word_id = ? AND normal_id = ? LIMIT 1`);
                const upd = db.prepare(`UPDATE word_to_normal_link SET weight = weight + 1 WHERE word_id = ? AND normal_id = ?`);
                const ins = db.prepare(`INSERT INTO word_to_normal_link (word_id, normal_id, weight) VALUES (?, ?, 1)`);
