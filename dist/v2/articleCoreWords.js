...
import { ArticleCoreWordsDataSchema, } from "./schemas.js";
...
function splitCompoundCoreWords(words) {
    const out = [];
    const seen = new Set();
...
function budgetUserNote(articleChars) {
    if (articleChars <= 0) {
        return "全文为空，无需输出锚点。";
    }
...
async function articleCoreWordsLlmFields(oc, strippedArticle, resolvedModel) {
    const budget = budgetUserNote(strippedArticle.length);
    const userBody = `${budget}\n\n--- 正文如下 ---\n\n${strippedArticle}`;
    const messagesParse = [
        { role: "system", content: SYSTEM_PROMPT_ARTICLE_CORE_WORDS },
    ...
    const messagesJson = [
        ...
        { role: "user", content: userBody + JSON_MODE_USER_SUFFIX_ARTICLE_CORE_WORDS },
    ];
...
export async function analyzeArticleCoreWords(text, opts) {
    loadProjectEnv();
