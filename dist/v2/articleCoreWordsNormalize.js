...
import { ArticleCoreWordsNomalizedDataSchema, } from "./schemas.js";
...
function normalizeOutputTokenBudget() {
...
function effectiveNormalizeOutputBudget(forDeepseek) {
...
function uniqueCoreWordsOrdered(coreWords) {
    const seen = new Set();
    const out = [];
...
function canonicalizeNewText(newText, originalText) {
    let nt = newText.trim();
    if (!nt) {
...
function mergeLlmWithCoverage(llm, orderedUniques) {
...
async function articleCoreWordsNormalizeLlm(oc, payload, resolvedModel) {
    const userBody = `以下为 core_words（JSON，已按首次出现顺序去重）。请输出 nomalized，规则见系统提示。\n${JSON.stringify(payload, null, 0)}`;
    const messagesParse = [
        { role: "system", content: SYSTEM_PROMPT_ARTICLE_CORE_WORDS_NORMALIZE },
    ...
    const messagesJson = [
        ...
        { role: "user", content: userBody + JSON_MODE_USER_SUFFIX_ARTICLE_CORE_WORDS_NORMALIZE },
    ];
...
export async function normalizeArticleCoreWordsSynonyms(data, opts) {
